...
export async function POST(req: NextRequest) {
  ...
  if (!account || account.provider !== "imap") {
    return NextResponse.json({ error: "IMAP account required for SMTP send in v0.1" }, { status: 400 });
  }
  ...
  // Header-injection defense: reject control characters in any header field.
